        ...
        <main>
          ...
          <article id="layout" data-title="界面布局 导航 功能在哪里">
            ...
            <dl className="map">
              ...
              <dt>实时曲线</dt>
              <dd>
                位于中央上方。可拖动底部把手改变高度，上限随可用空间调整，并为数据交互保留空间；收起后为数据交互释放空间。
              </dd>
              <dt>数据交互</dt>
          ...
          <article
            id="plot"
            data-title="实时曲线 绘图 数值 配色 PID 暂停 Y轴 缩放 双游标 区间统计 平均值 峰峰值 计算通道"
          >
            <h2>实时曲线</h2>
            <p>
            ...
            <ul>
              ...
              <li>
                移动鼠标查看最近采样点的实际值；缺测显示为空，不做插值。图例中的复选框可单独隐藏通道。
              </li>
              <li>
                曲线独立保留最多 100,000
                点，清空或暂停文本日志不会丢失曲线历史。设置中可选点序号或接收时间横轴、直线或阶梯连线。电脑接收时间不等于设备采样时间。
              </li>
              <li>
                Y 自动默认持续包含完整峰值；手动拖动或缩放 Y 轴后固定范围，再点“Y
                自动”恢复。可在设置中选择“抑制异常”，按两端各 1% 裁剪量程，但不修改原始采样和统计。
              </li>
              <li>Y 轴与区间统计以完整十进制数值显示；数据窗口的曲线通道显示窗口名和字段名。</li>
              <li>
                点击“双游标”打开独立系统测量窗口，不暂停接收或冻结波形。A/B
                跟随曲线滚动，保持采样点间隔，统计实时更新。窗口可移动、缩放，默认不置顶，点击“启用置顶
                / 取消置顶”切换。拖动 A / B，或先选择 A / B
                后点击图中位置；也可输入采样点编号，聚焦游标后用左右方向键逐点移动、Home / End
                移至首末点。两游标可交叉或重合。按住两游标之间的着色区域可整体平移，保持采样点间隔及顺序不变，到达数据首尾时整体停止；聚焦该区域后可用左右方向键或
                Home / End 平移。
              </li>
              <li>
                区间统计包含 A、B
                两端，按每个通道的有效原始样本计算最小值、最大值、算术平均值和峰峰值（最大值减最小值），不受绘图降采样影响。缺测不计入有效点数；没有有效值时显示“—”。Δt
                与 Δ值均为 B−A。
              </li>
              <li>
                测量窗口的“暂停接收 /
                继续接收”与主界面同步，暂停新增文本记录和曲线采样，保留数据及游标；端口保持打开，继续后不补录暂停期间的数据。关闭测量窗口或再次点击“双游标”结束测量。改变绘图点数、接收端口或应用计算程序会结束测量，清空曲线会同时清除游标。
              </li>
              <li>
                设置 → 计算通道可启用编程模式，process(data)
                接收原始通道数值，返回与数据窗口相同的结果数组，原始和计算通道合计最多 8
                个。程序按批处理全部采样，超时或积压时显示错误并保留原始值；修改程序需重新应用，会清空旧曲线。隐藏
                AD 通道可单独查看 gf 量程。
              </li>
          ...
          <article id="data-windows" data-title="数据窗口 置顶 自定义 匹配 模板 HEX 字节">
            ...
            <ul>
              ...
              <li>
                在“数据处理”中选择“编程模式”，编写 process(data)，通过 {'data["字段名"]'}{' '}
                读取已按符号和 DEC 小数位解析的数值。返回结果数组，例如
                {
                  ' [{ name: "压力", value: (data["数据"] - 1000) * 500 / 4000, unit: "gf", decimals: 2 }]'
                }
                。示例表示空载 AD 为 1000、500 gf 标定时 AD 为 5000，须按实际传感器标定值修改。
                支持多个字段和多个结果，原始 HEX / DEC
                仍保留。程序仅处理最新数据，高频时合并待处理值，
                不用于逐帧累计；超出安全整数范围、无效结果或执行超时会显示错误。
              </li>
              <li>
                点击“保存并应用”后保留配置及程序，下次从窗口列表重新打开。关闭主窗口会关闭所有数据窗口。
              </li>
            </ul>
          </article>
