      ...
      <div className="shell">
        ...
        <main>
          ...
          >
            ...
            <h3>接收分帧</h3>
            <p>
              点击标题栏中“条件暂停”右侧的“接收分帧”展开设置。所有串口共用分帧方式、分隔符、帧头帧尾和空闲时间；只有固定长度按串口分别设置。修改即时生效，并随工程导入导出。旧配置和工程以第一个串口的规则作为全局规则，各串口固定长度保持不变。
            </p>
            <ul>
              <li>
          ...
          <article id="data-windows" data-title="数据窗口 置顶 自定义 匹配 模板 HEX 字节">
            ...
            <ul>
              <li>
                勾选“显示到曲线图”，可选择 DEC 或 HEX。DEC 使用字段的小数位缩放，HEX
                使用未缩放的整数，两者均保留有符号设置；例如有符号 FF 85、小数位 2，DEC 绘制
                -1.23，HEX 绘制 -123。坐标轴仍显示十进制数值。选择随窗口配置保存，旧配置默认
                DEC；编程模式绘制程序返回的换算结果，不使用此选择。
              </li>
          ...
          <article id="pairs" data-title="虚拟串口对 创建 删除 驱动 UAC">
            <h2>虚拟串口对</h2>
            <p>
              用于创建两个互相连通的虚拟 COM 端口，适合联调两个串口程序。左侧 TAB
              列表显示已创建的串口对，可刷新状态或点击对应条目的删除按钮；侧栏支持折叠和拖拽调宽，与其他
              TAB
              一致。右侧选择两个未占用端口号后创建，创建和删除会自动刷新列表；端点不完整时禁用删除。
            </p>
            <div className="warn">
          ...
          >
            ...
            <h3>固件烧录</h3>
            <p>
              Ymodem 的 C 握手、应答、文件信息包和结束控制字节会显示在“数据交互”区，可用 ASCII/Hex
              按钮切换已有记录的显示格式。ASCII 模式只显示可读的烧录通讯，控制字节和二进制帧可切到
              Hex
              查看。固件数据包不会显示；烧录过程日志仍可在“固件烧录”页签查看。STM32CubeProgrammer 和
              esptool 由独立进程管理串口，应用无法取得工具内部的原始收发字节。烧录通讯不计入普通串口
              RX/TX 次数，也不会绘制到曲线。
            </p>
            <p>
          ...
          <article id="modbus" data-title="Modbus RTU 主站 从站 功能码 寄存器 CRC">
            <h2>Modbus RTU</h2>
            <p>
              拖动快捷指令栏右侧分隔条可调整宽度，自动记住设置；最小宽度与其他 TAB 内容区一致，为
              330px。双击分隔条恢复默认宽度。
            </p>
            <p>
            ...
            <p>
              左侧“快捷指令”用于状态切换、启停和运动控制，与自动回复规则页一样使用右键菜单管理：
              空白处右键可直接添加指令或分组；新指令可选择已有分组，默认保存到“未分组”。
              分组上右键添加指令、重命名或删除分组；指令上右键编辑或删除。 无需先创建设备配置。
              名称仅用于展示，点击右侧“写入”按钮执行，使用“通信设置”中的串口、从站地址和字序。
              指令包含名称、寄存器地址、数据和数据类型，支持 HEX16、UINT16、INT32、UINT32、FLOAT32。
              拖动 ⋮⋮ 调整分组或指令顺序，指令可拖入其他分组。地址和整数数据支持十进制或 0x
              前缀十六进制。
            </p>
            <p>
              主页寄存器的地址、命名和数据类型无论来自外部导入还是手动编辑，都会自动保存在本机，切换页面或重启后恢复。寄存器数值不持久化，由设备读取获取。
              左侧二级 TAB
              可在“快捷指令”和“配置管理器”间切换，共用可拖拽宽度，切换保留各自内容和配置选择。
              “配置管理器”中的右键“保存配置”直接保存主页当前已读取或已确认写入的寄存器地址、数据类型、数值、从站地址和字序，不弹出窗口。
              名称自动使用当前映射名称和从站地址，重名时追加序号。未读取的数据不会补零；暂无数据、32
              位数据不完整或非有限数值时会提示先读取有效数据。
              配置行右键可删除；点击配置整行任意位置选择配置，再应用并写入。
              快捷指令和设备配置独立保存在本机，切换或删除设备配置不会修改快捷指令。
              连接串口后选择配置，点击“应用并写入配置”按列表顺序执行，
              或点击单条指令的“写入”。写入采用所选配置的从站地址和字序，串口参数仍由串口配置页管理。
              批量写入会停止轮询，逐条等待确认；失败、断线或点击停止后不再发送后续指令。
              已写入数据不会回滚，超时或断线时应读回核对。停止按钮会等待当前指令响应。
              设备配置与原有寄存器映射 JSON 独立保存，不包含在工程或映射导出中。
            </p>
            <div className="tip">
