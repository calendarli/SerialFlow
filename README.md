...
点击“**双游标**”冻结波形后，拖动 A、B 或输入采样点位置，可测量时间差与数值差，并查看两游标之间各通道的**最小值、最大值、平均值和峰峰值**。统计使用包含两端的原始有效采样，允许游标交叉或重合；接收继续，点击“结束测量并继续”恢复实时波形。清空或暂停文本交互记录不影响曲线缓存。PID 分析提供响应指标和建议，不会自动写入设备。
