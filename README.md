...
点击“**双游标**”冻结波形后，拖动 A、B 或输入采样点位置，可测量时间差与数值差，并查看两游标之间各通道的**最小值、最大值、平均值和峰峰值**。统计使用包含两端的原始有效采样，允许游标交叉或重合。测量窗口的“暂停接收 / 继续接收”与主界面同步，暂停时停止新增文本记录和曲线采样，保留已有数据及游标；端口保持打开，继续后不补录暂停期间的数据，曲线以断点分隔。清空文本交互记录不影响曲线缓存。PID 分析提供响应指标和建议，不会自动写入设备。

区间测量在独立系统窗口中显示，不占用曲线区域，可移动、缩放。默认不置顶，通过“启用置顶 / 取消置顶”按钮切换；窗口内修改 A/B 采样点与主窗口曲线同步。关闭测量窗口或再次点击“双游标”会结束测量并恢复实时曲线。
