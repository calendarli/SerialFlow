...
Modbus RTU 左侧快捷指令支持拖动边界调整宽度、空白处右键直接新建指令、选择分组和拖拽排序；
...
实时曲线可识别 `PWM=10,Speed=20` 或 `10,20,30` 等数值格式，独立保留最多 100,000 个采样点，支持时间/点序号横轴、直线/阶梯线、完整峰值自动量程及计算通道（例如 AD 换算 gf）。缺测和断线处断开连线，绘图降采样保留峰谷。

曲线与数据显示区默认各占可用高度的 50%。拖动分隔条可调整并记住高度，双击恢复 50%；最大高度随窗口空间调整，并为数据显示区保留空间。

点击“**双游标**”打开测量窗口，不会暂停接收或冻结波形。A/B 跟随曲线滚动，保持相对当前视窗的位置与采样点间隔；统计随新数据实时更新，包括缓存满后的滚动更新。拖动 A、B 或输入采样点位置，可测量时间差与数值差，并查看两游标之间各通道的**最小值、最大值、平均值和峰峰值**。统计使用包含两端的原始有效采样，允许游标交叉或重合。测量窗口的“暂停接收 / 继续接收”与主界面同步，暂停时停止新增文本记录和曲线采样，保留已有数据及游标；端口保持打开，继续后不补录暂停期间的数据，曲线以断点分隔。清空文本交互记录不影响曲线缓存。PID 分析提供响应指标和建议，不会自动写入设备。

区间测量在独立系统窗口中显示，不占用曲线区域，默认尺寸为 680 × 656，可移动、缩放。默认不置顶，通过“启用置顶 / 取消置顶”按钮切换；窗口内修改 A/B 采样点与主窗口曲线同步。关闭测量窗口或再次点击“双游标”会结束测量，保留当前接收暂停状态。

按住 A/B 之间的着色区域可整体平移双游标，保持采样点间隔及 A/B 顺序不变；到达数据首尾时整体停止，区间统计同步更新。聚焦该区域后可用左右方向键逐点平移，Home / End 移至数据首尾。
